...
class ai_agent:
    ...
    def _backtrack(self, assignment):
        if (assignment.game_is_finished()
            and
            assignment.check_consistent):
        ...
        if limited_cell is not None:
            cell_included_limitation = assignment.extract_cells_for_limitation(limited_cell)

            value_sets = self.get_all_state(limited_cell, cell_included_limitation, assignment)
            for value_set in value_sets:
                if self.is_consistent(limited_cell, deepcopy(value_set), deepcopy(assignment)):
    ...
    def selected_unassigned_limited(self,
                               assignment):
        ...
        for limited_cell in assignment.limited_cells:
            if not assignment.is_limited_cell_assigned(limited_cell):
                unassigned_count = assignment.counter_limited_cell_unassigned(limited_cell)  
    ...
    def get_all_state(self, 
        ...
        for cell in cell_included_limitation:
            if cell.value == 0:unassigned_cells.append(cell)
        ...
        unassigned_value_sets = self.get_difference_way_for_sum(this_sum_now, cell_counter, _allowed_numbers)
    ...
    def get_difference_way_for_sum(self, 
                                   this_sum_now,
                                   cell_counter,
                                   allowed_numbers) -> list:
        if (cell_counter == 1 
            and 
            this_sum_now in allowed_numbers):
            return [[this_sum_now]]
        ...
        for value in allowed_numbers:
            ...
            if this_sum_now - value > 0:
                result = [[value] + combo for combo in self.get_difference_way_for_sum(this_sum_now - value, cell_counter - 1, allowed_numbers_copy)]
