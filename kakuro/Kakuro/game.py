...
class game:
    ...
    def create_board_geame(self) -> list:
        board = [[white_cell((i, j)) for j in range(self.size_x)]
    ...
    def extract_cells_for_limitation(self,
                         limited_cell) -> list:
        ...
        if limited_cell.ways is self.ways["DOWN"]:
            for i in range(limited_cell.size):
                cell_included_limitation.append(
                    self.board[limited_cell.location[0] + i + 1][limited_cell.location[1]]
                )
        ...
        if limited_cell.ways is self.ways["RIGHT"]:
            for i in range(limited_cell.size):
                cell_included_limitation.append(
                    self.board[limited_cell.location[0]][limited_cell.location[1] + i + 1]
                )
        
        return cell_included_limitation
    
    def counter_limited_cell_unassigned(self,
                                        limited_cell) -> int:
        
        counter = 0
        for cell in self.extract_cells_for_limitation(limited_cell):
            if cell.value == 0:counter += 1
        return counter
    
    def is_limited_cell_assigned(self,
                                 limited_cell) -> bool:
        ...
        for limited_cell in self.limited_cells:
            ...
            if self.is_limited_cell_assigned(limited_cell):
                ...
                for element in consistent_cells:
                    ...
                    Sum += element.value
                if (Sum != limited_cell.target_sum
                    or
                    self._check_repeat(values)):
                    return False
    ...
    def game_is_finished(self) -> bool:
        for i in range(0, self.size_y):
            for j in range(0, self.size_x):
                if (self.board[i][j].category == self.detail["WHITE"]
                    and 
    ...
    def assign_limited_cell(self,
                            limited_cell,
                            values : list) -> None:
        
        if limited_cell.ways is self.ways["DOWN"]:
            for i in range(limited_cell.size):
                self.board[limited_cell.location[0] + i + 1][limited_cell.location[1]].value = values.pop(0)
                
        elif limited_cell.ways is self.ways["RIGHT"]:
            for i in range(limited_cell.size):
                self.board[limited_cell.location[0]][limited_cell.location[1] + i + 1].value = values.pop(0)
